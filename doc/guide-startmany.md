...
### Send 10000 ADVC to New Addresses

Send exactly 10000 ADVC to each new address created above.
...
Copy the masternode private key and corresponding collateral output transaction that holds the 10000 ADVC.

*Note: The masternode private key is **not** the same as a wallet private key. **Never** put your wallet private key in the masternode.conf file. That is almost equivalent to putting your 10000 ADVC on the remote server and defeats the purpose of a hot/cold setup.*
...
[From the advc github repo](https://github.com/advcpay/advc/blob/master/doc/masternode_conf.md)
...
## Update advc.conf on server

If you generated a new masternode private key, you will need to update the remote `advc.conf` files.

Shut down the daemon and then edit the file.

```nano .advccore/advc.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `advc.conf` file.
...
and comparing with the official explorer at https://explorer.advc.org/chain/Adventurecoin
...
```
advc-cli masternode status
{
    "outpoint" : "<collateral_output>-<collateral_output_index>",
    "service" : "<ipaddress>:<port>",
    "pubkey" : "<10000 ADVC address>",
...
Search your Masternodes on https://advcninja.pl/masternodes.html
