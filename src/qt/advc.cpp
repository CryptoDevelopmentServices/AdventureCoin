...
{
    return QCoreApplication::translate("advc-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in advc.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in advc.qrc)
...
/** Class encapsulating Adventurecoin Core startup and shutdown.
...
/** Main Adventurecoin application object */
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Adventurecoin Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(advc);
    Q_INIT_RESOURCE(advc_locale);
    ...
    /// 6. Determine availability of data directory and parse advc.conf
    ...
    if(!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("Adventurecoin Core"),
    ...
    // advc: links repeatedly have their payment requests routed to this process:
